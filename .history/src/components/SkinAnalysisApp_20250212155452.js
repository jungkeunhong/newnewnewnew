...
const SkinAnalysisApp = () => {
  ...
  const quizQuestions = [
    ...
    {
      id: 8,
      question: "Please enter your email (required)",
      type: "email",
      required: true
    }
  ];
  ...
  const SkinQuizScreen = ({ onComplete }) => {
    ...
    const saveAnswersToFirebase = async (answers) => {
      try {
        ...
        const dataToSave = {
          ethnicity: answers[0] ? answers[0] : '미지정',
          ageGroup: answers[1] ? answers[1] : '미지정',
          skinType: answers[2] ? answers[2] : '미지정',
          skinConcern: answers[3] ? answers[3] : '미지정',
          productPreference: answers[4] ? answers[4] : '미지정',
          dailySkincareTime: answers[5] ? answers[5] : '미지정',
          makeupFrequency: answers[6] ? answers[6] : '미지정',
          email: answers[7] ? answers[7] : '',
    ...
    const analyzeQuizResults = (answers) => {
      return {
        ...
        ageGroup: answers[1] ? answers[1] : '미지정',
        skinType: answers[2] ? answers[2] : '미지정',
        concerns: answers[3] ? [answers[3]] : ['미지정'],
        recommendations: {
          treatments: ["맞춤형 트리트먼트", "전문가 상담"],
          products: ["맞춤형 세럼", "맞춤형 크림"]
        }
      };
    };

    // 현재 질문과 답변이 변경될 때마다 localStorage에 저장
    useEffect(() => {
        ...
        if (validateEmail(answer)) {
          saveAnswersToFirebase(newAnswers)
            .then(() => {
              ...
              const results = analyzeQuizResults(newAnswers);
              onComplete(results);
            })
            .catch((error) => {
              console.error('퀴즈 답변 저장 중 오류:', error);
    ...
    // 현재 질문이 유효한지 확인
    if (currentQuestion < 0 || currentQuestion >= quizQuestions.length || !quizQuestions[currentQuestion]) {
      return null;
    }
      ...
      >
        <div className="max-w-md mx-auto space-y-6">
          <div className="text-center space-y-2">
            <h1 className="text-2xl font-bold text-luxe-900">맞춤형 피부 분석</h1>

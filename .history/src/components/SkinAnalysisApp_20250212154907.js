...
const SkinAnalysisApp = () => {
  ...
  const SkinQuizScreen = ({ onComplete }) => {
    ...
    const handleEmailSubmit = () => {
      const email = answers[currentQuestion];
    ...
    const saveAnswersToFirebase = async (answers) => {
      try {
        ...
        const dataToSave = {
          ...
          ageGroup: answers[1] || '',
          skinType: answers[2] || '',
          skinConcern: answers[3] || '',
          productPreference: answers[4] || '',
          dailySkincareTime: answers[5] || '',
          makeupFrequency: answers[6] || '',
          email: answers[7] || '',
    ...
    if (!currentQuestionObj) {
      console.error('현재 질문을 찾을 수 없습니다:', currentQuestion);
      return null;
    }

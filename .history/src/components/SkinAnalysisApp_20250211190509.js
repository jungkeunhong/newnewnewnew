...
const SkinAnalysisApp = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      const hasCompletedQuiz = localStorage.getItem(`quiz_completed_${user.uid}`);
      if (hasCompletedQuiz) {
  ...
  const handleQuizComplete = (results) => {
    // Save quiz results
    localStorage.setItem(`quiz_completed_${user.uid}`, 'true');
    localStorage.setItem(`quiz_results_${user.uid}`, JSON.stringify(results));
    setQuizCompleted(true);
    setCurrentTab('search');
  };
  ...
  const LoginScreen = () => (
    ...
    >
      <div className="w-full max-w-md space-y-8 text-center">
        ...
        >
          <h1 className="text-4xl font-bold gotu-regular text-luxe-900 mb-4">
            Asentica
          </h1>
          <p className="text-luxe-600 mb-8">
            Find your perfect skin clinic with personalized AI recommendations
          </p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-center space-x-4">
            <div className="w-12 h-12 rounded-full bg-luxe-50 flex items-center justify-center">
          ...
          <div className="text-center space-y-2">
            <p className="font-medium text-luxe-900">Join thousands of satisfied users</p>
      ...
      {showMessage && (
        ...
        >
          <p className="text-center text-luxe-500">
            {user ? "📸 고민 부위(이마, 코, 볼 등)를 선택하여 찍어주세요!" : "Please sign in to analyze your skin 🔒"}
          </p>
        </motion.div>
      )}
            ...
            >
              <div className="space-y-4">
                <div className="p-4 bg-luxe-50 rounded-xl">
                  <h3 className="font-medium text-luxe-900 mb-2">📸 사진 촬영 가이드</h3>
                  <ul className="space-y-2 text-sm text-luxe-600">
                    <li className="flex items-center">
                      <span className="mr-2">🎯</span>
                      <span>고민 부위(이마, 코, 볼 등)를 선택하여 클로즈업 촬영</span>
                    </li>
                    <li className="flex items-center">
                      <span className="mr-2">🌟</span>
                      <span>전반적인 분석을 원하면 밝은 환경에서 전체 얼굴 촬영</span>
                    </li>
                    <li className="flex items-center">
                      <span className="mr-2">🧹</span>
                      <span>카메라 렌즈의 먼지를 닦고 촬영</span>
                    </li>
                  </ul>
                </div>

                <div className="p-4 bg-luxe-50 rounded-xl">
                  <h3 className="font-medium text-luxe-900 mb-2">🔒 개인정보 보호</h3>
                  <p className="text-sm text-luxe-600">
                    귀하의 사진은 피부 분석 용도로만 사용되며, 절대 저장되지 않습니다.
                  </p>
                </div>
              </div>
            </motion.div>
  ...
  const SearchScreen = () => {
    const [selectedFilters, setSelectedFilters] = useState({
      ethnicity: user?.quizResults?.ethnicity || 'all',
      skinType: user?.quizResults?.skinType || 'all',
      skinConcern: user?.quizResults?.skinConcern || 'all',
      ageGroup: user?.quizResults?.ageGroup || 'all'
    });
                ...
                {user && (
                  <div className="flex items-center space-x-2">
                    ...
                    <button 
                      ...
                      className="text-sm text-luxe-400 hover:text-luxe-600"
                    >
                      Logout
                    </button>
                  </div>

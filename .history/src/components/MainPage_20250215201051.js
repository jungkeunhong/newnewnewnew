...
import { Camera, Star, ChevronRight, ChevronLeft, Droplet, Sun, Heart, Frown, Smile } from 'lucide-react';
...
const MainPage = ({ quizResults, onStartAnalysis, onTreatmentClick, onBack, onDoctorClick }) => {
  ...
  const getConcernEmoji = (concern) => {
    ...
    return emojiMap[concern.toLowerCase()] || '✨';
  };
    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="cormorant text-2xl">피부 분석 결과</h2>
          <div className="text-4xl">{getSkinTypeEmoji(skinType)}</div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-[#F8F6F4] rounded-xl p-4">
            <div className="flex items-center space-x-2 mb-2">
              <Droplet className="w-5 h-5 text-[#3E2723]" />
              <h3 className="font-medium">피부 타입</h3>
            </div>
          ...
          <div className="bg-[#F8F6F4] rounded-xl p-4">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h3 className="font-medium">주요 피부 고민</h3>
            </div>
            <div className="flex flex-wrap gap-2">
              {concerns.slice(0, 3).map((concern, index) => (
                <span key={index} className="inline-flex items-center bg-white px-3 py-1 rounded-full text-sm">

...
const SkinAnalysisApp = () => {
  ...
  const [step, setStep] = useState(() => {
    ...
    // If we have a saved image and the step is detailed, restore it
    if (savedImage && savedStep === 'detailed') {
  ...
  // Save selectedImage to localStorage when it changes
  useEffect(() => {
    if (selectedImage) {
  ...
  useEffect(() => {
    if (user && !loading) {
      ...
      if (redirectStep === 'detailed' && savedImage) {
        setSelectedImage(savedImage);
        setStep('detailed');
        // Clean up URL

  ...
  const SkinQuizScreen = ({ onComplete }) => {
    ...
    const handleAnswer = (answer) => {
      ...
      if (currentQuestion < questions.length - 1) {
        ...
        saveAnswersToAirtable(newAnswers);
    ...
    const saveAnswersToAirtable = async (answers) => {
      try {
        const response = await fetch('https://api.airtable.com/v0/YOUR_BASE_ID/Quiz%20Responses', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            fields: {
              'Skin Type': answers[0],
              'Skin Concern': answers[1],
              'Product Preference': answers[2],
              'Daily Skincare Time': answers[3],
              'Makeup Frequency': answers[4],
              'Email': answers[5],
        ...
        <div className="max-w-md mx-auto space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-luxe-600">
              Please answer a few questions for more accurate analysis
            </p>
            ...
            {questions[currentQuestion].type === 'email' ? (
              <div className="space-y-3">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
                    if (answers[currentQuestion]?.includes('@')) {

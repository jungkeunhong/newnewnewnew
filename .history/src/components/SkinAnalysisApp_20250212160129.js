  ...
  const SkinQuizScreen = ({ onComplete }) => {
    ...
    const saveAnswersToFirebase = async (answers) => {
      try {
        ...
        const dataToSave = {
          ethnicity: answers[0] || '미지정',
          ageGroup: answers[1] || '미지정',
          skinType: answers[2] || '미지정',
          skinConcern: answers[3] || '미지정',
          productPreference: answers[4] || '미지정',
          dailySkincareTime: answers[5] || '미지정',
          makeupFrequency: answers[6] || '미지정',
          email: answers[7] || '',
      ...
      if (currentQuestion < quizQuestions.length - 1) {
        ...
        if (currentQuestion === quizQuestions.length - 1) {
          if (validateEmail(answer)) {
            const results = {
              ethnicity: newAnswers[0] || '미지정',
              ageGroup: newAnswers[1] || '미지정',
              skinType: newAnswers[2] || '미지정',
              concerns: [newAnswers[3] || '미지정'],
              recommendations: {
